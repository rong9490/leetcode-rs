...
impl Solution {
    pub fn garbage_collection(garbage: Vec<String>, travel: Vec<i32>) -> i32 {
        // vec包含4个长度: 空, 'M', 'P', 'G', 
        let (mut ret, mut sum, mut cnt) = (garbage[0].len() as i32, 0, vec![0; 4]);

        // 从左往右, 记录每种垃圾最后位置来计算行驶时间, 避免hastSet登记;
        // 从1开始, 第二个房子计算清理时间
        for i in 1..garbage.len() {
            let arr = garbage[i].bytes().collect::<Vec<_>>();
            ret += arr.len() as i32;
            sum += travel[i - 1];
            for chr in arr {
                // 分品类累计时间, usize & 3 取模
                ret += sum - cnt[chr as usize & 3]; // 减去多的偏差
                cnt[chr as usize & 3] = sum;
            }
        }
        ret
    }
