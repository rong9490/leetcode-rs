...
impl Solution {
    pub fn min_cost_to_move_chips(position: Vec<i32>) -> i32 {
        // 贪心: 只分两种情况, 码在偶数位置 / 码在奇数位置
        // 所有奇数位置花费为0, 偶数花费为1

        let even_cost: i32 = position.iter().filter(|v| (**v & 1) == 0).count() as i32;
        let odd_cost: i32 = position.iter().filter(|v| (**v & 1) == 1).count() as i32;
        even_cost.min(odd_cost)
    }
}
// @lc code=end
