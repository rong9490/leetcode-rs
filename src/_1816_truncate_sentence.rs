...
impl Solution {
    pub fn truncate_sentence(s: String, k: i32) -> String {
        // 按空格(whitespace)分隔单词, 自动执行trim()的
        let words: Vec<&str> = s.split_whitespace().collect::<Vec<&str>>();
        // get访问更安全
        let front_words: Vec<&str> = (0..k).map(|idx| *words.get(idx as usize).unwrap()).collect::<Vec<_>>();
        // 拼接 Vec<&str> -> String
        let sentence: String = front_words.join(" ");
        sentence
    }
}
// @lc code=end
